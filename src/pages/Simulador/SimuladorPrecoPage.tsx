...
export const SimuladorPrecoPage = ({ navigateTo, pratoInicial, onSave, isTestMode }: SimuladorPrecoPageProps) => {
    ...
    const [ingredientes, setIngredientes] = useState<Ingrediente[]>(pratoInicial?.ingredientes || [{ id: Date.now(), nome: 'Frango (Filé)', precoCompra: 20, qtdUsada: 0.2, undUsada: 'kg' }]);
    ...
    const handleAddIngrediente = () => {
        setIngredientes([...ingredientes, { id: Date.now(), nome: '', precoCompra: 0, qtdUsada: 0, undUsada: 'kg' }]);
    };
    ...
    const calculos = useMemo(() => {
        const custoTotalIngredientes = ingredientes.reduce((acc, ing) => acc + (ing.precoCompra * ing.qtdUsada), 0);
        ...
        const precoDeVendaSugerido = divisor > 0 ? custoTotalSemApp / divisor : custoTotalSemApp;
        ...
        return { custoTotalIngredientes, custoIndiretoPorPrato, custoTotalSemApp, precoDeVendaSugerido, lucroBruto, margemReal, composicaoCustoData };
    }, [ingredientes, custoEmbalagem, taxaApp, custosIndiretosMensais, pratosVendidosMes, margemLucroDesejada, maoDeObra, isModoEvento, custosEvento]);
                                ...
                                {ingredientes.map((ing) => (
                                    <div key={ing.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center border-b pb-4">
                                        ...
                                        <div className="md:col-span-3 flex items-center">
                                            ...
                                            <input type="number" placeholder="Preço" value={ing.precoCompra} onChange={e => handleIngredienteChange(ing.id, 'precoCompra', parseFloat(e.target.value) || 0)} className="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <input type="number" placeholder="Qtd." value={ing.qtdUsada} onChange={e => handleIngredienteChange(ing.id, 'qtdUsada', parseFloat(e.target.value) || 0)} className="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md" />
                                        </div>
                                        <div className="md:col-span-2">
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie data={calculos.composicaoCustoData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={60} fill="#8884d8">
                                            {calculos.composicaoCustoData.map((_entry, index) => (
